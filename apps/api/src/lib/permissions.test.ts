...
describe('§7 resolution order', () => {
  it('step 1 — a superadmin holds everything without any grants', async () => {
    ...
    expect(hasPermission({ ...access, userId: id, tokenVersion: 0, agentId: null, customerId: null, isExternal: false }, 'ACCOUNTS.BALANCE_SHEET.VIEW')).toBe(true);
    expect(hasPermission({ ...access, userId: id, tokenVersion: 0, agentId: null, customerId: null, isExternal: false }, 'ANYTHING.AT.ALL')).toBe(true);
  });
  ...
  it('step 4 — DENY does NOT beat superadmin', async () => {
    ...
    const access = await resolve(id);
    expect(access.isSuperadmin).toBe(true);
    expect(hasPermission({ ...access, userId: id, tokenVersion: 0, agentId: null, customerId: null, isExternal: false }, VIEW)).toBe(true);
  });
