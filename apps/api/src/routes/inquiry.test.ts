...
describe('the Required container grid picks a size (client request, 2026-09-06)', () => {
  /*
   * "Container size" was a free-text box in every mode. It matters most on a
   * Consol Box, where the column reads CBM and nothing said whether the box is
   * a 20ft or a 40ft; typed by hand it was also unreportable, since "40HC",
   * "40 HC" and "40' High Cube" are three strings for one box.
   *
   * The screen now offers the container-size master and stores the chosen
   * name, which is what these guard: the column still takes text, so the
   * contract did not change and every inquiry already keyed in still reads.
   */
  it('stores the size chosen against a consol box', async () => {
    const response = await create(tokenAdmin, {
      loadingType: 'CONSOL_BOX',
      volumes: [{ volumeKind: 'LCL', cbm: '24.000', containerSizeNote: "40' High Cube" }],
    });
    expect(response.status, JSON.stringify(response.body.error ?? {})).toBe(201);
    expect(response.body.data.volumes[0].containerSizeNote).toBe("40' High Cube");
  });

  it('leaves it null when nothing was picked', async () => {
    // The picker's first option is "—", and an empty string is not a size.
    const response = await create(tokenAdmin, {
      loadingType: 'CONSOL_BOX',
      volumes: [{ volumeKind: 'LCL', cbm: '10.000', containerSizeNote: '' }],
    });
    expect(response.status, JSON.stringify(response.body.error ?? {})).toBe(201);
    expect(response.body.data.volumes[0].containerSizeNote).toBeNull();
  });

  it('puts the box on the rate request an agent is asked to price', async () => {
    /*
     * The agent email renders the volume itself. Without the box, an agent
     * pricing a consol box would be told the cubic metres and left to guess
     * whether that is going in a 20ft or a 40ft — which is the difference
     * between two quite different prices.
     */
    const created = await create(tokenAdmin, {
      loadingType: 'CONSOL_BOX',
      volumes: [{ volumeKind: 'LCL', cbm: '24.000', containerSizeNote: "20' Standard" }],
    });
    expect(created.status, JSON.stringify(created.body.error ?? {})).toBe(201);

    const stored = await owner.inquiryVolume.findFirstOrThrow({
      where: { inquiryId: BigInt(created.body.data.id), deletedAt: null },
      select: { cbm: true, containerSizeNote: true, volumeKind: true },
    });
    expect(stored.volumeKind).toBe('LCL');
    expect(stored.containerSizeNote).toBe("20' Standard");
    expect(stored.cbm?.toString()).toBe('24');
  });
});
